...
$comments_delete = NULL; // Initialize page object first

class ccomments_delete extends ccomments {
	...
	function Page_Main() {
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("commentslist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($comments_delete)) $comments_delete = new ccomments_delete();
...
var CurrentForm = fcommentsdelete = new ew_Form("fcommentsdelete", "delete");

// Form_CustomValidate event
fcommentsdelete.Form_CustomValidate = 
...
fcommentsdelete.ValidateRequired = true;
<?php } else { ?>
fcommentsdelete.ValidateRequired = false; 
<?php } ?>
...
?>
<form name="fcommentsdelete" id="fcommentsdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($comments_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $comments_delete->Token ?>">
<?php } ?>
...
<?php if ($comments->commentID->Visible) { // commentID ?>
		<th><span id="elh_comments_commentID" class="comments_commentID"><?php echo $comments->commentID->FldCaption() ?></span></th>
<?php } ?>
<?php if ($comments->username->Visible) { // username ?>
		<th><span id="elh_comments_username" class="comments_username"><?php echo $comments->username->FldCaption() ?></span></th>
<?php } ?>
<?php if ($comments->mediaid->Visible) { // mediaid ?>
		<th><span id="elh_comments_mediaid" class="comments_mediaid"><?php echo $comments->mediaid->FldCaption() ?></span></th>
<?php } ?>
<?php if ($comments->dateCreated->Visible) { // dateCreated ?>
		<th><span id="elh_comments_dateCreated" class="comments_dateCreated"><?php echo $comments->dateCreated->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
...
<?php } ?>
<?php if ($comments->username->Visible) { // username ?>
		<td<?php echo $comments->username->CellAttributes() ?>>
<span id="el<?php echo $comments_delete->RowCnt ?>_comments_username" class="comments_username">
...
<?php } ?>
<?php if ($comments->mediaid->Visible) { // mediaid ?>
		<td<?php echo $comments->mediaid->CellAttributes() ?>>
<span id="el<?php echo $comments_delete->RowCnt ?>_comments_mediaid" class="comments_mediaid">
...
<?php } ?>
<?php if ($comments->dateCreated->Visible) { // dateCreated ?>
		<td<?php echo $comments->dateCreated->CellAttributes() ?>>
<span id="el<?php echo $comments_delete->RowCnt ?>_comments_dateCreated" class="comments_dateCreated">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $comments_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fcommentsdelete.Init();
</script>
<?php
