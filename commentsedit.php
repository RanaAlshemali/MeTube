...
$comments_edit = NULL; // Initialize page object first

class ccomments_edit extends ccomments {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->commentID->CurrentValue == "")
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->commentID->FldIsDetailKey)
			$this->commentID->setFormValue($objForm->GetValue("x_commentID"));
...
// Create page object
if (!isset($comments_edit)) $comments_edit = new ccomments_edit();
...
var CurrentForm = fcommentsedit = new ew_Form("fcommentsedit", "edit");
...
fcommentsedit.Form_CustomValidate = 
...
fcommentsedit.ValidateRequired = true;
<?php } else { ?>
fcommentsedit.ValidateRequired = false; 
<?php } ?>
...
?>
<form name="fcommentsedit" id="fcommentsedit" class="<?php echo $comments_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($comments_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $comments_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="comments">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<div>
<?php if ($comments->commentID->Visible) { // commentID ?>
	<div id="r_commentID" class="form-group">
		<label id="elh_comments_commentID" class="col-sm-2 control-label ewLabel"><?php echo $comments->commentID->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $comments->commentID->CellAttributes() ?>>
<span id="el_comments_commentID">
<span<?php echo $comments->commentID->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $comments->commentID->EditValue ?></p></span>
</span>
<input type="hidden" data-table="comments" data-field="x_commentID" name="x_commentID" id="x_commentID" value="<?php echo ew_HtmlEncode($comments->commentID->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $comments_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
