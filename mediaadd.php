...
$media_add = NULL; // Initialize page object first

class cmedia_add extends cmedia {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "mediaview.php")
	...
	function LoadDefaultValues() {
		...
		$this->duration->CurrentValue = "00:00";
		$this->privacy->CurrentValue = "public";
		$this->catagory->CurrentValue = "People & Blogs";
	}
	...
	function AddRow($rsold = NULL) {
		...
		// duration
		$this->duration->SetDbValueDef($rsnew, $this->duration->CurrentValue, "", strval($this->duration->CurrentValue) == "");

		// privacy
		$this->privacy->SetDbValueDef($rsnew, $this->privacy->CurrentValue, "", strval($this->privacy->CurrentValue) == "");

		// catagory
		$this->catagory->SetDbValueDef($rsnew, $this->catagory->CurrentValue, "", strval($this->catagory->CurrentValue) == "");
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$rsnew['mediaid'] = $this->mediaid->DbValue;
			}
		} else {
...
// Create page object
if (!isset($media_add)) $media_add = new cmedia_add();
...
var CurrentForm = fmediaadd = new ew_Form("fmediaadd", "add");
...
fmediaadd.ValidateRequired = true;
<?php } else { ?>
fmediaadd.ValidateRequired = false; 
<?php } ?>
...
?>
<form name="fmediaadd" id="fmediaadd" class="<?php echo $media_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($media_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $media_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $media_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
