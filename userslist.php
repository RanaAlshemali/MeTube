...
class cusers_list extends cusers {
	...
	function GetFilterList() {
		...
		$sFilterList = ew_Concat($sFilterList, $this->avatar->AdvancedSearch->ToJSON(), ","); // Field avatar
		$sFilterList = ew_Concat($sFilterList, $this->signup_date->AdvancedSearch->ToJSON(), ","); // Field signup_date
		if ($this->BasicSearch->Keyword <> "") {
	...
	function RestoreFilterList() {
		...
		// Field signup_date
		$this->signup_date->AdvancedSearch->SearchValue = @$filter["x_signup_date"];
		$this->signup_date->AdvancedSearch->SearchOperator = @$filter["z_signup_date"];
		$this->signup_date->AdvancedSearch->SearchCondition = @$filter["v_signup_date"];
		$this->signup_date->AdvancedSearch->SearchValue2 = @$filter["y_signup_date"];
		$this->signup_date->AdvancedSearch->SearchOperator2 = @$filter["w_signup_date"];
		$this->signup_date->AdvancedSearch->Save();
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->signup_date); // signup_date
	...
	function LoadRowValues(&$rs) {
		...
		$this->signup_date->setDbValue($rs->fields('signup_date'));
	}
	...
	function LoadDbValues(&$rs) {
		...
		$this->signup_date->DbValue = $row['signup_date'];
	}
	...
	function RenderRow() {
		...
		// signup_date
		$this->signup_date->ViewValue = $this->signup_date->CurrentValue;
		$this->signup_date->ViewCustomAttributes = "";
			...
			// signup_date
			$this->signup_date->LinkCustomAttributes = "";
			$this->signup_date->HrefValue = "";
			$this->signup_date->TooltipValue = "";
		}
...
<?php if ($users->_email->Visible) { // email ?>
	...
	<?php } ?>
<?php } ?>		
<?php if ($users->signup_date->Visible) { // signup_date ?>
	<?php if ($users->SortUrl($users->signup_date) == "") { ?>
		<th data-name="signup_date"><div id="elh_users_signup_date" class="users_signup_date"><div class="ewTableHeaderCaption"><?php echo $users->signup_date->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="signup_date"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $users->SortUrl($users->signup_date) ?>',1);"><div id="elh_users_signup_date" class="users_signup_date">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $users->signup_date->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($users->signup_date->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($users->signup_date->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
<?php echo $users->_email->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($users->signup_date->Visible) { // signup_date ?>
		<td data-name="signup_date"<?php echo $users->signup_date->CellAttributes() ?>>
<span id="el<?php echo $users_list->RowCnt ?>_users_signup_date" class="users_signup_date">
<span<?php echo $users->signup_date->ViewAttributes() ?>>
<?php echo $users->signup_date->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
