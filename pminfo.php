...
class cpm extends cTable {
	...
	function __construct() {
		...
		// title
		$this->title = new cField('pm', 'pm', 'x_title', 'title', '`title`', '`title`', 201, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['title'] = &$this->title;

		// user1
		$this->user1 = new cField('pm', 'pm', 'x_user1', 'user1', '`user1`', '`user1`', 200, -1, FALSE, '`user1`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->user1->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['user1'] = &$this->user1;

		// user2
		$this->user2 = new cField('pm', 'pm', 'x_user2', 'user2', '`user2`', '`user2`', 200, -1, FALSE, '`user2`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->user2->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['user2'] = &$this->user2;

		// message
		$this->message = new cField('pm', 'pm', 'x_message', 'message', '`message`', '`message`', 201, -1, FALSE, '`message`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fields['message'] = &$this->message;

		// timestamp
		$this->timestamp = new cField('pm', 'pm', 'x_timestamp', 'timestamp', '`timestamp`', 'DATE_FORMAT(`timestamp`, \'%Y/%m/%d\')', 135, -1, FALSE, '`timestamp`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->timestamp->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['timestamp'] = &$this->timestamp;

		// user1read
		$this->user1read = new cField('pm', 'pm', 'x_user1read', 'user1read', '`user1read`', '`user1read`', 200, -1, FALSE, '`user1read`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['user1read'] = &$this->user1read;

		// user2read
		$this->user2read = new cField('pm', 'pm', 'x_user2read', 'user2read', '`user2read`', '`user2read`', 200, -1, FALSE, '`user2read`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['user2read'] = &$this->user2read;

		// messageID
		$this->messageID = new cField('pm', 'pm', 'x_messageID', 'messageID', '`messageID`', '`messageID`', 3, -1, FALSE, '`messageID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`pm`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("pmview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("pmview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "pmadd.php?" . $this->UrlParm($parm);
		else
			$url = "pmadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("pmedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("pmadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("pmdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->user2->EditValue = $this->user2->CurrentValue;
		...
		$this->user2read->EditValue = $this->user2read->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->user1->Exportable) $Doc->ExportCaption($this->user1);
					if ($this->user2->Exportable) $Doc->ExportCaption($this->user2);
					if ($this->message->Exportable) $Doc->ExportCaption($this->message);
					if ($this->timestamp->Exportable) $Doc->ExportCaption($this->timestamp);
					if ($this->user1read->Exportable) $Doc->ExportCaption($this->user1read);
					if ($this->user2read->Exportable) $Doc->ExportCaption($this->user2read);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->user1->Exportable) $Doc->ExportField($this->user1);
						if ($this->user2->Exportable) $Doc->ExportField($this->user2);
						if ($this->message->Exportable) $Doc->ExportField($this->message);
						if ($this->timestamp->Exportable) $Doc->ExportField($this->timestamp);
						if ($this->user1read->Exportable) $Doc->ExportField($this->user1read);
						if ($this->user2read->Exportable) $Doc->ExportField($this->user2read);
	...
	function Recordset_Selecting(&$filter) {
		...
		$sFilter = "user1 = '" . CurrentUserInfo("username") . "'";
