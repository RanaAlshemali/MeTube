...
class cPMs extends cTable {
	...
	function __construct() {
		...
		// ID
		$this->ID = new cField('PMs', 'PMs', 'x_ID', 'ID', '`ID`', '`ID`', 20, -1, FALSE, '`ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ID'] = &$this->ID;

		// usrfrom
		$this->usrfrom = new cField('PMs', 'PMs', 'x_usrfrom', 'usrfrom', '`usrfrom`', '`usrfrom`', 200, -1, FALSE, '`usrfrom`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['usrfrom'] = &$this->usrfrom;

		// usrto
		$this->usrto = new cField('PMs', 'PMs', 'x_usrto', 'usrto', '`usrto`', '`usrto`', 200, -1, FALSE, '`usrto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['usrto'] = &$this->usrto;

		// date
		$this->date = new cField('PMs', 'PMs', 'x_date', 'date', '`date`', 'DATE_FORMAT(`date`, \'%Y/%m/%d\')', 135, 5, FALSE, '`date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->date->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['date'] = &$this->date;

		// title
		$this->title = new cField('PMs', 'PMs', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['title'] = &$this->title;

		// body
		$this->body = new cField('PMs', 'PMs', 'x_body', 'body', '`body`', '`body`', 201, -1, FALSE, '`body`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`PMs`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("PMsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("PMsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "PMsadd.php?" . $this->UrlParm($parm);
		else
			$url = "PMsadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("PMsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("PMsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("PMsdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->usrto->EditValue = $this->usrto->CurrentValue;
		$this->usrto->PlaceHolder = ew_RemoveHtml($this->usrto->FldCaption());

		// date
		$this->date->EditAttrs["class"] = "form-control";
		$this->date->EditCustomAttributes = "";
		$this->date->EditValue = ew_FormatDateTime($this->date->CurrentValue, 5);
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->usrfrom->Exportable) $Doc->ExportCaption($this->usrfrom);
					...
					if ($this->usrfrom->Exportable) $Doc->ExportCaption($this->usrfrom);
					if ($this->usrto->Exportable) $Doc->ExportCaption($this->usrto);
					if ($this->date->Exportable) $Doc->ExportCaption($this->date);
					if ($this->title->Exportable) $Doc->ExportCaption($this->title);
				}
				$Doc->EndExportRow();
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->usrfrom->Exportable) $Doc->ExportField($this->usrfrom);
						...
						if ($this->usrfrom->Exportable) $Doc->ExportField($this->usrfrom);
						if ($this->usrto->Exportable) $Doc->ExportField($this->usrto);
						if ($this->date->Exportable) $Doc->ExportField($this->date);
						if ($this->title->Exportable) $Doc->ExportField($this->title);
					}
					$Doc->EndExportRow();
	...
	function Recordset_Selecting(&$filter) {
		...
		$sFilter = "usrto = '" . CurrentUserInfo("username") . "' AND usrfrom NOT IN (SELECT blocklistee from blockList where blockList.blocklister = '" . CurrentUserInfo("username") . "')";
	...
	///$sql=mysql_query("SELECT FROM users WHERE username=$rsnew["usrto"]);
