...
class ccomments extends cTable {
	...
	function __construct() {
		...
		// commentID
		$this->commentID = new cField('comments', 'comments', 'x_commentID', 'commentID', '`commentID`', '`commentID`', 3, -1, FALSE, '`commentID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->commentID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['commentID'] = &$this->commentID;

		// content
		$this->content = new cField('comments', 'comments', 'x_content', 'content', '`content`', '`content`', 201, -1, FALSE, '`content`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fields['content'] = &$this->content;

		// username
		$this->username = new cField('comments', 'comments', 'x_username', 'username', '`username`', '`username`', 200, -1, FALSE, '`username`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['username'] = &$this->username;

		// mediaid
		$this->mediaid = new cField('comments', 'comments', 'x_mediaid', 'mediaid', '`mediaid`', '`mediaid`', 3, -1, FALSE, '`mediaid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->mediaid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['mediaid'] = &$this->mediaid;

		// dateCreated
		$this->dateCreated = new cField('comments', 'comments', 'x_dateCreated', 'dateCreated', '`dateCreated`', 'DATE_FORMAT(`dateCreated`, \'%Y/%m/%d\')', 135, 5, FALSE, '`dateCreated`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`comments`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('commentID', $rs))
				ew_AddFilter($where, ew_QuotedName('commentID', $this->DBID) . '=' . ew_QuotedValue($rs['commentID'], $this->commentID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->commentID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@commentID@", ew_AdjustSql($this->commentID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("commentsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("commentsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "commentsadd.php?" . $this->UrlParm($parm);
		else
			$url = "commentsadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("commentsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("commentsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "commentID:" . ew_VarToJson($this->commentID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->commentID->CurrentValue)) {
			$sUrl .= "commentID=" . urlencode($this->commentID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["commentID"]))
				$arKeys[] = ew_StripSlashes($_POST["commentID"]);
			elseif (isset($_GET["commentID"]))
				$arKeys[] = ew_StripSlashes($_GET["commentID"]);
			else
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->commentID->Exportable) $Doc->ExportCaption($this->commentID);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->commentID->Exportable) $Doc->ExportField($this->commentID);
