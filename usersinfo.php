...
class cusers extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('users', 'users', 'x_id', 'id', '`id`', '`id`', 20, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// avatar
		$this->avatar = new cField('users', 'users', 'x_avatar', 'avatar', '`avatar`', '`avatar`', 201, -1, FALSE, '`avatar`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fields['avatar'] = &$this->avatar;

		// signup_date
		$this->signup_date = new cField('users', 'users', 'x_signup_date', 'signup_date', '`signup_date`', '`signup_date`', 3, -1, FALSE, '`signup_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->signup_date->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['signup_date'] = &$this->signup_date;
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		// signup_date
		$this->signup_date->EditAttrs["class"] = "form-control";
		$this->signup_date->EditCustomAttributes = "";
		$this->signup_date->EditValue = $this->signup_date->CurrentValue;
		$this->signup_date->PlaceHolder = ew_RemoveHtml($this->signup_date->FldCaption());
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->signup_date->Exportable) $Doc->ExportCaption($this->signup_date);
				} else {
					if ($this->id->Exportable) $Doc->ExportCaption($this->id);
					if ($this->username->Exportable) $Doc->ExportCaption($this->username);
					if ($this->password->Exportable) $Doc->ExportCaption($this->password);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->signup_date->Exportable) $Doc->ExportField($this->signup_date);
					} else {
						if ($this->id->Exportable) $Doc->ExportField($this->id);
						if ($this->username->Exportable) $Doc->ExportField($this->username);
						if ($this->password->Exportable) $Doc->ExportField($this->password);
